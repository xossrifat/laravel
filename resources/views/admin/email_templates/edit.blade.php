    ...
    <div class="bg-white dark:bg-gray-800 shadow-md rounded-lg overflow-hidden p-6">
        <form method="POST" action="{{ route('admin.email_templates.update', $template->id) }}">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $template->name) }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline dark:bg-gray-700 dark:border-gray-600 dark:text-white @error('name') border-red-500 @enderror" required>
                @error('name')
            ...
            <div class="mb-4">
                <label class="block text-gray-700 text-sm font-bold mb-2 dark:text-gray-300">Template Slug:</label>
                <div class="text-gray-600 dark:text-gray-400 mb-2">{{ $template->slug }}</div>
                <p class="text-gray-500 text-xs mt-1 dark:text-gray-400">The slug is automatically generated and used to identify this template in code.</p>
            </div>
            
            <div class="mb-4">
                <label for="subject" class="block text-gray-700 text-sm font-bold mb-2 dark:text-gray-300">Email Subject:</label>
                <input type="text" name="subject" id="subject" value="{{ old('subject', $template->subject) }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline dark:bg-gray-700 dark:border-gray-600 dark:text-white @error('subject') border-red-500 @enderror" required>
                @error('subject')
                    <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-4">
                ...
                <select name="type" id="type" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline dark:bg-gray-700 dark:border-gray-600 dark:text-white @error('type') border-red-500 @enderror" required>
                    ...
                    <option value="reward" {{ old('type', $template->type) == 'reward' ? 'selected' : '' }}>Reward</option>
            ...
            <div class="mb-4">
                ...
                <textarea name="content" id="content" rows="15" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline dark:bg-gray-700 dark:border-gray-600 dark:text-white @error('content') border-red-500 @enderror" required>{{ old('content', $template->content) }}</textarea>
                @error('content')
                    <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-6">
                <label for="active" class="flex items-center">
                    <input type="checkbox" name="active" id="active" value="1" {{ old('active', $template->active) ? 'checked' : '' }} class="mr-2">
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <a href="{{ route('admin.email_templates.preview', $template->id) }}" target="_blank" class="ml-2 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">
                        Preview
                    </a>
                </div>
