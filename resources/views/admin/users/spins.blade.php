...
@section('title', 'User Spins History')
@section('header', 'Spins History')

@section('content')
<div class="container mx-auto px-4 sm:px-6 lg:px-8">
    <div class="sm:flex sm:items-center">
        <div class="sm:flex-auto">
            <h1 class="text-2xl font-semibold text-gray-900">Spins History</h1>
            <p class="mt-2 text-sm text-gray-700">
                Spin history for {{ $user->name }} ({{ $user->email }})
            </p>
        </div>
    </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-300">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Coins Won</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200 bg-white">
                            ...
                            <tr>
                                <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm text-gray-500 sm:pl-6">
                                    {{ $spin->created_at->format('M d, Y H:i:s') }}
                                </td>
                                <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                                    {{ $spin->reward->label ?? 'Unknown Reward' }}
                                </td>
                                <td class="whitespace-nowrap px-3 py-4 text-sm font-medium text-green-600">
                                    +{{ number_format($spin->coins_won) }}
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="3" class="px-3 py-4 text-sm text-gray-500 text-center">No spins found</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
